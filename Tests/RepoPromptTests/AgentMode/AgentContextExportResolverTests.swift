...
final class AgentContextExportResolverTests: XCTestCase {
    ...
    func testNonGitAutomaticExportBatchesSelectedPathLookupsAndReportsUnavailableCodemapCoverage() async throws {
        #if DEBUG
            ...
            XCTAssertGreaterThan(runtimeAccessCount.value, 0)
            XCTAssertLessThanOrEqual(runtimeAccessCount.value, explicitFileCount)
            XCTAssertEqual(
    ...
    func testMetadataOnlyWorktreeMissingPathUsesLogicalDisplayPath() async throws {
        let fixture = try await makeBoundFixture()
        let source = makeSource(
            logicalRoot: fixture.logicalRoot,
            worktreeRoot: fixture.worktreeRoot,
            selection: StoredSelection(selectedPaths: ["Sources/Missing.swift"], codemapAutoEnabled: false)
        )
        ...
        XCTAssertEqual(model.missingPaths, ["Sources/Missing.swift"])
        XCTAssertFalse(model.missingPaths.contains { $0.contains(fixture.worktreeRoot.path) })
    }
    ...
    func testSelectedManualCodemapOnlySelectionProducesCodemapRow() async throws {
        let repositoryFixture = try ReviewGitRepositoryFixture(name: #function)
        defer { repositoryFixture.cleanup() }
        let root = try repositoryFixture.makeRepository(
            named: "agent-export-selected-manual-only",
            files: ["Sources/App.swift": "struct ManualOnlyCodemap { func selectedOnlySymbol() {} }\n"]
        )
        let store = try makeIsolatedCodemapStore(name: #function)
        _ = try await store.loadRoot(path: root.path)
        let source = AgentContextExportSource(
        ...
        XCTAssertEqual(row.kind, .codemap)
        XCTAssertEqual(row.displayPath, "Sources/App.swift")
        XCTAssertFalse(model.codemapPresentation.orderedEntries.isEmpty)
        XCTAssertTrue(model.missingPaths.isEmpty)
        XCTAssertTrue(model.invalidPaths.isEmpty)
    }
    ...
    func testSelectedUnavailableCodemapPreservesFullRowAndReportsIncompleteCoverage() async throws {
        ...
        XCTAssertFalse(model.codemapIssues.isEmpty)
        XCTAssertEqual(runtimeAccessCount.value, 1)
    }
