...
class LifecycleQueueTests(LifecycleTestCase):
    ...
    def test_test_operations_default_to_bounded_ci_timeout(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _focused_argv, _focused_lanes, _focused_cwd, _focused_env, focused_timeout = registry.prepare(
                {"operation": "test", "args": {"filter": "PiRPCClientTests"}}
            )
    ...
    def test_root_tests_retry_one_suite_without_xctest_startup_output_without_skip_build(self) -> None:
        ...
        completed = subprocess.CompletedProcess(
            ["swift", "test", "list"],
            0,
            stdout="RepoPromptTests.SilentStartupTests/testCase\n",
            stderr="",
        )
    ...
    def test_streaming_silent_startup_guard_kills_process_group_descendants(self) -> None:
        ...
        descendant_script = "import signal, time; signal.signal(signal.SIGTERM, signal.SIG_IGN); time.sleep(30)"
        parent_script = """
import subprocess
import sys
import time
