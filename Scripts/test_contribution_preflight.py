...
class ContributionPreflightTests(unittest.TestCase):
    ...
    def exported_stub_function(self, log_env_name: str) -> str:
        return f"() {{ printf '%s\\n' \"$*\" >> \"${{{log_env_name}:?}}\"; }}"
    ...
    def create_repo(self, root: Path, *, outgoing_path: str | None = None) -> tuple[Path, Path, dict[str, str]]:
        ...
        env["BASH_FUNC_make%%"] = self.exported_stub_function("RPCE_STUB_MAKE_LOG")
        env["BASH_FUNC_gitleaks%%"] = self.exported_stub_function("RPCE_STUB_GITLEAKS_LOG")
        env["TMPDIR"] = str(root)
