...
Focused validation, release validation, and local-production install validation remain explicit; use the validation matrix plus commands such as `make dev-release-preflight` / `make dev-release-artifact` / `CONFIRM_LOCAL_PRODUCTION_INSTALL=1 make dev-install-local-production` when the changed boundary requires them. Obtain explicit user approval immediately before any force-push, history rewrite, branch deletion, fork deletion, credential rotation, other GitHub-visible destructive mutation, visible app launch/relaunch, or stopping a visible app.
