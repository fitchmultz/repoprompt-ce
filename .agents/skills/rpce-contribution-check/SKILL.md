...
description: Validate RepoPrompt CE contributions before committing or pushing. Use whenever an agent is about to create a commit, push the current branch, prepare PR-ready local evidence, rewrite history, delete a branch or fork, or change GitHub-visible repository state. Enforces staged-index and outgoing-range secret scanning, repository guardrails, clean push boundaries, an explicit PR-ready lane for path-selected heavyweight validation, and explicit approval for destructive Git or visible live-app operations.
---
...
4. Read [references/validation-matrix.md](references/validation-matrix.md) and ensure any required focused, release, smoke, local-production, or PR-ready evidence is recorded before pushing.
...
`pr-ready` reruns the push safety gate, then runs any matching path-selected heavyweight lanes for the computed outgoing range, such as conductor selftests, Swift lint, root/provider tests, and product builds. It does not replace explicit release validation, local-production install validation, live smoke, already-pushed PR-base comparison, or destructive-operation approval requirements.
...
Read [references/validation-matrix.md](references/validation-matrix.md) when deciding whether additional focused tests, builds, PR-ready validation, release checks, local-production checks, or live smoke are required.
