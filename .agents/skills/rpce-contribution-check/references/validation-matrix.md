...
Use this after the mandatory safety preflight when the touched boundary needs focused, PR-ready, release, local-production, or live-app evidence.
...
| Local-production / MitchPrompt CE install changes | Run `make dev-install-local-production` or `make install-local-production` only with the documented confirmation environment and explicit owner approval. Local-production install lanes are not part of default `push` or `pr-ready`. |
