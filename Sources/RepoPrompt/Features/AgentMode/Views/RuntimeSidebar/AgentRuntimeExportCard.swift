...
@MainActor
private enum AgentSelectedFilesModelPlumbing {
    static func makeExportSource(
    ...
    static func makeModelRequest(
        ...
        activeAgentSessionID: UUID?,
        worktreeBindingsProvider: (@MainActor (UUID, UUID?) -> [AgentSessionWorktreeBinding])?,
        activePopoverTab: AgentSelectedFilesPopoverTab,
        flushPendingUI: Bool = true
        ...
        let source = makeExportSource(
            ...
            flushPendingUI: flushPendingUI
        )
        let cfg = promptManager.resolvePromptContext(BuiltInCopyPresets.standard, custom: nil)
        let codeMapUsage = activePopoverTab == .codemaps ? cfg.codeMapUsage : .none
    ...
    static func remove(
        _ row: AgentContextExportRow,
        from model: AgentContextExportModel,
        promptManager: PromptViewModel,
        selectionCoordinator: WorkspaceSelectionCoordinator?,
        ...
        Task {
            let latestSelection = latestSelection(
                for: model.source,
                promptManager: promptManager,
                selectionCoordinator: selectionCoordinator
            )
            ...
            await persistSelection(updated, source: model.source, selectionCoordinator: selectionCoordinator)
            await MainActor.run { refresh() }
        }
    }

    static func clearSelection(
        for model: AgentContextExportModel,
        promptManager: PromptViewModel,
        selectionCoordinator: WorkspaceSelectionCoordinator?,
        refresh: @escaping @MainActor () -> Void
    ) {
...
struct AgentSelectedFilesPopoverTrigger<Label: View>: View {
    ...
    private var selectionSummary: AgentContextSelectionSummary {
        ...
        return AgentContextExportResolver.selectionSummary(
            for: AgentSelectedFilesModelPlumbing.makeExportSource(
                promptManager: promptManager,
                selectionCoordinator: selectionCoordinator,
                currentTabID: currentTabID,
                activeAgentSessionID: activeAgentSessionID,
                worktreeBindingsProvider: worktreeBindingsProvider,
                flushPendingUI: false
            ).selection
        )
    }
    ...
    private func makeModelRequest(flushPendingUI: Bool = true) -> AgentSelectedFilesModelRequest {
        AgentSelectedFilesModelPlumbing.makeModelRequest(
            component: "trigger",
            promptManager: promptManager,
            selectionCoordinator: selectionCoordinator,
            currentTabID: currentTabID,
            activeAgentSessionID: activeAgentSessionID,
            worktreeBindingsProvider: worktreeBindingsProvider,
            activePopoverTab: activePopoverTab,
            flushPendingUI: flushPendingUI
        )
    }
    ...
    private func loadRowContent(
        ...
        await AgentSelectedFilesModelPlumbing.loadRowContent(
            row,
            model: model,
            promptManager: promptManager,
            purpose: purpose
    ...
    private func remove(_ row: AgentContextExportRow, from model: AgentContextExportModel) {
        AgentSelectedFilesModelPlumbing.remove(
            row,
            from: model,
            promptManager: promptManager,
            selectionCoordinator: selectionCoordinator
        ) {
            refreshExportModel(force: true)
        }
    }

    private func clearSelection(for model: AgentContextExportModel) {
        AgentSelectedFilesModelPlumbing.clearSelection(
            for: model,
            promptManager: promptManager,
            selectionCoordinator: selectionCoordinator
        ) {
