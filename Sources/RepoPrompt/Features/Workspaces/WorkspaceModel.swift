...
struct StoredSelection: Codable, Equatable, Hashable {
    ...
    init(from decoder: Decoder) throws {
        ...
        manualCodemapPaths = try container.decodeIfPresent([String].self, forKey: .manualCodemapPaths) ?? []
        slices = try container.decodeIfPresent([String: [LineRange]].self, forKey: .slices) ?? [:]
        codemapAutoEnabled = try container.decodeIfPresent(Bool.self, forKey: .codemapAutoEnabled) ?? true
    }
