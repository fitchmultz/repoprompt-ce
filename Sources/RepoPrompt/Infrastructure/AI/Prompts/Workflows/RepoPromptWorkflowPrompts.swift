...
enum RepoPromptWorkflowPrompts {
    ...
    /// Version 62: rp-orchestrate — forbids Agent Mode orchestrators from spawning child agents by shelling out to external CLIs (`pi -p`, `codex`, `claude`, `rpce-cli`, etc.); delegated work must stay in RepoPrompt-managed `agent_run` sessions so it remains visible, steerable, cancellable, and bound to the current app context.
